...
export default class CustomParamView extends Vue {
  model: CustomParam = { parameterName: "", explanation: "Custom Param", min: 0, max: 1, defaultValue: 0 };
  ...
  async submit() {
    ...
    try {
      let newParamRes = await plugin.apiClient.parameterCreation(this.model);
      ...
      ElMessage.error(
        {
          message: `参数: ${this.model.parameterName} 创建失败!\n ${e.message}`
        }
      );
    } finally {
      loading.close();
